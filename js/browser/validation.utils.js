...
(function (global, factory) {
  ...
  class WellidateValidatable {
      ...
      reset(message) {
          ...
          validatable.element.dispatchEvent(
              new CustomEvent("wellidate-reset", {
                  detail: { validatable },
                  bubbles: true
              })
          );
      ...
      pending(message) {
          ...
          this.element.dispatchEvent(
              new CustomEvent("wellidate-pending", {
      ...
      success(message) {
          ...
          validatable.element.dispatchEvent(
              new CustomEvent("wellidate-success", {
                  detail: { validatable },
                  bubbles: true
              })
          );
      ...
      error(method, message) {
          ...
          validatable.element.dispatchEvent(
              new CustomEvent("wellidate-error", {
                  detail: {
      ...
      buildErrorContainers() {
          ...
          if (name) {
              ...
              this.errorContainers = [].map.call(
                  document.querySelectorAll(`[data-valmsg-for="${name}"]`),
                  (container) => container
              );
          } else {
      ...
      buildInputRules() {
          ...
          if (element.getAttribute("maxlength") && defaultRules.maxlength) {
              rules.maxlength = Object.assign(
                  {},
                  defaultRule,
                  defaultRules.maxlength,
                  {
      ...
      buildDataRules() {
          ...
          const defaultRule = Wellidate.default.rule;
          const attributes = [].filter.call(element.attributes, (attribute) =>
              /^data-val-\w+$/.test(attribute.name)
          );
          ...
          for (const attribute of attributes) {
              ...
              const rule =
                  this.rules[method] || Wellidate.default.rules[method];
              ...
              if (rule) {
                  ...
                  [].forEach.call(element.attributes, (parameter) => {
                      if (parameter.name.indexOf(`${prefix}-`) == 0) {
                          dataRule[
                              parameter.name.substring(prefix.length + 1)
                          ] = parameter.value;
                      }
                  });

                  this.rules[method] = Object.assign(
                      {},
                      defaultRule,
                      rule,
                      dataRule,
      ...
      bind() {
          ...
          const event =
              input.tagName == "SELECT" || input.type == "hidden"
                  ? "change"
                  : "input";
  ...
  class Wellidate {
      constructor(container, options = {}) {
          ...
          if (container.dataset.valId) {
              return Wellidate.instances[
                  parseInt(container.dataset.valId)
              ].set(options);
          }

          wellidate.wasValidatedClass =
              Wellidate.default.classes.wasValidated;
          wellidate.inputPendingClass =
              Wellidate.default.classes.inputPending;
          wellidate.fieldPendingClass =
              Wellidate.default.classes.fieldPending;
      ...
      set(options) {
          ...
          for (const selector of Object.keys(options.rules || {})) {
              for (const validatable of wellidate.filterValidatables(
                  selector
              )) {
                  const element = validatable.element;
                  ...
                  for (const method of Object.keys(options.rules[selector])) {
                      ...
                      const methodRule =
                          validatable.rules[method] ||
                          Wellidate.default.rules[method] ||
                          {};

                      validatable.rules[
                          method
                      ] = wellidate.extend(defaultRule, methodRule, newRule, {
                          element
                      });
                  }
              }
      ...
      rebuild() {
          ...
          if (wellidate.container.matches(wellidate.include)) {
              ...
              wellidate.validatables.push(
                  new WellidateValidatable(wellidate, group)
              );
          } else {
              for (const element of wellidate.container.querySelectorAll(
                  wellidate.include
              )) {
      ...
      form(...filter) {
          ...
          if (wellidate.focusInvalid) {
              wellidate.focus(
                  result.invalid.map((invalid) => invalid.validatable)
              );
          }
      ...
      setOption(option, value) {
          ...
          if (typeof value != "undefined") {
              if (
                  ...
                  wellidate[option] = wellidate.extend(
                      wellidate[option],
                      value
                  );
              } else {
          ...
          if (errors.length) {
              ...
              for (let i = 1; i < errors.length; i++) {
                  if (this.lastActive == errors[i].element) {
                      ...
                      invalid.element.compareDocumentPosition(
                          errors[i].element
                      ) == 2
                  ) {
                      invalid = errors[i];
      ...
      bind() {
          ...
          if (wellidate.container.tagName == "FORM") {
              wellidate.container.addEventListener("submit", function (e) {
                  if (wellidate.form()) {
                      this.dispatchEvent(
                          new CustomEvent("wellidate-valid", {
                              detail: { wellidate },
                              bubbles: true
                          })
                      );
                      ...
                      this.dispatchEvent(
                          new CustomEvent("wellidate-invalid", {
                              detail: { wellidate },
                              bubbles: true
                          })
                      );
      ...
      rules: {
          ...
          equalto: {
              ...
              isValid() {
                  ...
                  return (
                      other != null &&
                      this.normalizeValue() == this.normalizeValue(other)
                  );
              }
          },
          length: {
              ...
              isValid() {
                  ...
                  return (
                      (length.min == null || length.min <= value.length) &&
                      (value.length <= length.max || length.max == null)
                  );
              },
              formatMessage() {
                  const length = this;
                  ...
                  if (
                      length.min != null &&
                      length.max == null &&
                      !length.isDataMessage
                  ) {
                      return Wellidate.default.rules.minlength.message.replace(
                          "{0}",
                          length.min
                      );
                  } else if (
                      ...
                      return Wellidate.default.rules.maxlength.message.replace(
                          "{0}",
                          length.max
                      );
                  }
          ...
          number: {
              ...
              scaleMessage:
                  "Please enter a value with no more than {0} fractional digits",
              precisionMessage:
                  "Please enter a value using no more than {0} significant digits",
              isValid() {
                  const number = this;
                  const scale = parseInt(number.scale);
                  const value = number.normalizeValue();
                  const precision = parseInt(number.precision);
                  let isValid = /^$|^[+-]?(\d+|\d{1,3}(,\d{3})+)?(\.\d+)?$/.test(
                      value
                  );

                  if (isValid && value && precision > 0) {
                      number.isValidPrecision =
                          number.digits(
              ...
              digits(value) {
                  return value.split("").filter((e) => !isNaN(parseInt(e)))
                      .length;
              },
              formatMessage() {
                  const number = this;
                  ...
                  if (
                      number.isValidPrecision === false &&
                      !number.isDataMessage
                  ) {
                      return number.precisionMessage.replace(
                          "{0}",
                          parseInt(number.precision) -
                              (parseInt(number.scale) || 0)
                      );
                  } else if (
                      number.isValidScale === false &&
                      !number.isDataMessage
                  ) {
                      return number.scaleMessage.replace(
                          "{0}",
                          parseInt(number.scale) || 0
                      );
                  }
          ...
          range: {
              ...
              formatMessage() {
                  ...
                  if (
                      ...
                      return Wellidate.default.rules.min.message.replace(
                          "{0}",
                          range.min
                      );
                  } else if (
                      ...
                      return Wellidate.default.rules.max.message.replace(
                          "{0}",
                          range.max
                      );
                  }
          ...
          filesize: {
              ...
              formatMessage() {
                  ...
                  return filesize.message.replace(
                      "{0}",
                      mb.replace(/[.|0]*$/, "")
                  );
              }
          ...
          remote: {
              ...
              buildUrl() {
                  ...
                  const fields = (remote.additionalFields || "")
                      .split(",")
                      .filter(Boolean);
                  ...
                  for (const field of fields) {
                      ...
                      url.searchParams.append(
                          element.name,
                          remote.normalizeValue(element) || ""
                      );
                  }

                  url.searchParams.append(
                      remote.element.name,
                      remote.normalizeValue() || ""
                  );
